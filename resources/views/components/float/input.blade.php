...
<div <?php echo $type == 'password' ? 'x-data="{show: false}"' : ''; ?> class="relative {{ $class }}">
    ...
    @if($type == 'password')
        <button type="button" x-cloak @click="show = !show" :title="show ? 'Hide' : 'Show'" class="absolute top-0 bottom-0 right-0 inline-flex items-center justify-center mx-1 my-1 px-2 py-1 rounded-md border bg-gray-100 text-gray-700 focus:outline-primary-500">
